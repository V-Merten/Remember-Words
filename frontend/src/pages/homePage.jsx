      ...
      <div className="page-layout">
        <div className="word-section">
          ...
          <table className="word-table">
            <thead>
              <tr>
                <th className='section-text'>Foreign Word</th>
                <th className='section-text'>Translation</th>
                <th>
                  <input
                    type="checkbox"
                    checked={allSelected}
          ...
          <div className="actions-container">
            <button onClick={() => handleDeleteSelected(selectedWords, setWords, words, setSelectedWords, setEditingWord, setEditValues, editingWord)} className='little-text'>Delete</button>
            <button onClick={() => handleUpdateSelected(selectedWords, words, setEditingWord, setEditValues)} className='little-text'>Rename</button>
            <select
              value={selectedGroupId}
              onChange={(e) => setSelectedGroupId(e.target.value)}
            >
              <option value="" className='little-text'>Select group</option>
              {groups.map((group) => (
                <option key={group.id} value={group.id}>
                  {group.name}
                </option>
            ...
            <button className='little-text'
              onClick={async () => {
                for (const wordId of selectedWords) {
                  await handleAddWordToGroup(wordId);
                }
              }}
              disabled={selectedWords.length === 0 || !selectedGroupId}
            >
              Assign to Group
            </button>
          </div>
